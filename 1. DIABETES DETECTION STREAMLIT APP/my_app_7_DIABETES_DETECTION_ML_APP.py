...
# For columns having values Yes/No
def get_fvalue(val):
...
def run_ml_app():
    ...
    with st.expander("Prediction Results"):
        ...
        if prediction == 1:
            st.error("Positive Risk - Model Predicts Class : {}".format(prediction[0]),icon='☠')
            pred_probability_score = {"Negative Diabetes Mellitus(DM)": str(round(pred_prob[0][0] * 100, 2)) + ' %',
                                      "Positive Diabetes Mellitus(DM)": str(round(pred_prob[0][1] * 100, 2)) + ' %'}
            st.subheader("Prediction Probability Score")
            st.json(pred_probability_score)
        else:
            st.success("Negative Risk - Model Predicts Class : {}".format(prediction[0]), icon='🛡️')
            pred_probability_score = {"Negative Diabetes Mellitus(DM)": str(round(pred_prob[0][0] * 100, 2)) + ' %',
                                      "Positive Diabetes Mellitus(DM)": str(round(pred_prob[0][1] * 100, 2))+' %'}
            st.subheader("Prediction Probability Score")
            st.json(pred_probability_score)
